...
#global shit
app = "youtube-dl"
path = "-o '$HOME/Music/youtube-dl_songs/%(channel)s/%(title)s - %(channel)s.%(ext)s'"
...
link = input()
os.system(f"{app} {path} {audiodownload} {link}")
os.system(f"{app} {path} {thumbnaildownload} {link}")
os.system(f"{app} {title} {link} {filename}")
# os.system("find ~/Downloads/youtube-dl_songs -name '*.webp' -exec magick {} {}.png \; -exec rm {} \;\n")
# os.system("sort -o $HOME/yt-download_history.txt $HOME/yt-download_history.txt")
    # f.close()
